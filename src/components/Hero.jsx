...
// NOTE: bumped base opacity + fill/border strength — these were only ever
// hitting full visibility on :hover, which never fires on touch devices,
// so mobile users were seeing near-invisible placeholders.
const SaasDashboard = () => (
  <div className="absolute inset-0 p-8 flex items-end justify-center pointer-events-none opacity-70 group-hover:opacity-100 transition-opacity duration-700">
    <div className="w-[85%] h-[60%] rounded-t-2xl bg-white/[0.04] border-t border-x border-white/15 p-5 flex flex-col gap-4 shadow-2xl relative overflow-hidden">
      <div className="absolute top-0 left-0 w-full h-1/2 bg-gradient-to-b from-cyan-500/15 to-transparent" />
      <div className="w-full h-4 rounded-full bg-white/10 relative z-10" />
      <div className="flex gap-4 h-full relative z-10">
        <div className="w-1/3 h-full rounded-lg bg-white/10" />
...
const AppWireframe = () => (
  <div className="absolute inset-0 p-8 flex flex-col items-end justify-start pointer-events-none opacity-70 group-hover:opacity-100 transition-opacity duration-700">
    <div className="w-1/2 h-[90%] rounded-[2rem] bg-white/[0.04] border border-white/15 p-4 flex flex-col gap-4 shadow-2xl relative overflow-hidden">
      <div className="absolute top-0 left-0 w-full h-1/2 bg-gradient-to-b from-[#d94814]/15 to-transparent" />
      <div className="w-1/3 h-2 mx-auto rounded-full bg-white/25 mb-2 relative z-10" />
      <div className="w-full h-12 rounded-xl bg-white/10 relative z-10" />
      <div className="w-3/4 h-6 rounded-xl bg-white/10 relative z-10" />
      <div className="w-full flex-1 rounded-t-xl bg-white/10 relative z-10 mt-auto" />
    </div>
  </div>
);
...
const capabilities = [
  {
    ...
    glowTheme: "cyan",
    // Cards with a background visual need room up top for it to peek through.
    // Cards without one don't — that empty 80px gap was the "why is there
    // dead space on this card" problem.
    contentMargin: "mt-12 md:mt-20",
    delay: 0.1,
  },
  {
    ...
    <div className="relative overflow-hidden">
      ...
      <section className="relative pt-32 md:pt-40 pb-16 px-6 max-w-screen-xl mx-auto">
        <div className="relative z-10 max-w-5xl">
          ...
          {/*
            Rotating headline container: heights now grow monotonically with
            the font size at each breakpoint (previously sm was SMALLER than
            base even though the text is bigger there — a layout mismatch
            that risked clipping/jumping between breakpoints).
          */}
          <h1 className="text-[2.75rem] sm:text-6xl md:text-[5.5rem] font-bold tracking-tighter leading-[1.1] mb-4 md:mb-6">
            ...
            <span
              className="block text-[#d94814] relative mt-1 md:mt-2 h-[78px] sm:h-[96px] md:h-[130px] w-full"
          ...
          {capabilities.map((item) => {
            ...
            return (
              <motion.div
                ...
                className={`group relative bg-[#0d0d10]/90 backdrop-blur-md border border-white/10 ${borderHover} rounded-3xl p-6 md:p-8 transition-all duration-500 overflow-hidden flex flex-col justify-end ${item.gridClass}`}
              >
                <div className={`absolute top-0 right-0 w-48 h-48 blur-[60px] pointer-events-none transition-colors duration-500 ${glowBg}`} />

                {Visual && <Visual />}

                <div className={`relative z-10 w-full max-w-[90%] ${item.contentMargin}`}>
                  <div className={`mb-6 w-12 h-12 rounded-xl bg-white/[0.06] border border-white/15 flex items-center justify-center text-white transition-all duration-300 ${iconGlow}`}>
